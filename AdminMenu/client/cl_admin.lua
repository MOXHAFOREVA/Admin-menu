...
Citizen.CreateThread(function()
	...
	WarMenu.SetSubTitle('MainMenu', 'Admin Menü')
	WarMenu.CreateSubMenu('SelfMenu', 'MainMenu', 'Ayarlar ~b~>~s~')
-- Vehicule mod
	WarMenu.CreateSubMenu('VehMenu', 'MainMenu', 'Harika ~b~>~s~')
	WarMenu.CreateSubMenu('weather', 'MainMenu', 'Hava Durumu ~b~>~s~')
-- Other	
	WarMenu.CreateSubMenu('PlayerMenu', 'MainMenu', 'Oyuncu Ayarları ~b~>~s~')
	WarMenu.CreateSubMenu('OnlinePlayerMenu', 'PlayerMenu', 'Online Oyuncu Menüsü ~b~>~s~')
	WarMenu.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Oyuncu Ayarları ~b~>~s~')
	WarMenu.CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Silah Menüsü ~b~>~s~')
	WarMenu.CreateSubMenu('SingleWepMenu', 'WepMenu', 'Silah Menüsü ~b~>~s~')
	WarMenu.CreateSubMenu('kickall', 'PlayerMenu', 'Herkesi Oyundan At ?')
	...
	while Enabled do
		if WarMenu.IsMenuOpened('MainMenu') then
			...
			DrawScaleformMovie(scaleform, 1.183, 0.6247, 0.9, 0.9, 255, 255, 255, 255, 0)
			if WarMenu.MenuButton('Admin Menüsü ~b~>~s~', 'SelfMenu') then
			elseif WarMenu.MenuButton('MUKEMMEL-MENU ~b~>~s~', 'VehMenu') then
			elseif WarMenu.MenuButton('Hava Durumu ~b~>~s~', 'weather') then
			elseif WarMenu.MenuButton('Digerler ~b~>~s~', 'PlayerMenu') then
		end
...
-- Self Menu			
		...
		if WarMenu.CheckBox("GÖRÜNMEZ OL",Invisible,function(enabled)
			Invisible = enabled
		end) then
		...
		--elseif WarMenu.Button('No Clip moins rapide ') then       EN DEV
			noclip_speed = noclip_speed - 0.3
		elseif WarMenu.Button("~h~~r~Araç Sil") then
		...
		elseif WarMenu.Button("Zırh Ver") then 
			SetPedArmour(PlayerPedId(), 200)
			TriggerServerEvent("logMenuAdmin", '\n- Se redonne de l\'armure( Heal )')
		elseif WarMenu.CheckBox('GOD', Godmode, function(enabled)
			Godmode = enabled
		end) then
		...
		elseif WarMenu.Button('Öldür') then
...
-- Véhicule Menu
		...
		elseif WarMenu.IsMenuOpened('VehMenu') then
			...
			elseif WarMenu.Button("~h~~r~Tüm Araçları Sil") then
				TriggerServerEvent('saksofon-dellallveh')
				TriggerServerEvent("logMenuAdmin", 'Delete de véhicule')
			elseif WarMenu.Button("~h~~r~Tüm Pedleri Sil") then
				TriggerServerEvent('saksofon-dellpeds')
				TriggerServerEvent("logMenuAdmin", 'Delete de véhicule')
			elseif WarMenu.Button("~h~~r~Topluluk Servisi Data Sil") then
				TriggerServerEvent('tplklk')
			elseif WarMenu.Button("~h~~r~Tüm Objeleri Sil") then
				TriggerServerEvent('saksofon-dellobject')
				TriggerServerEvent("logMenuAdmin", 'Delete de véhicule')
			end
			WarMenu.Display()
			DrawScaleformMovie(scaleform, 1.183, 0.6247, 0.9, 0.9, 255, 255, 255, 255, 0)
		...
		elseif WarMenu.IsMenuOpened('weather') then
			if WarMenu.MenuButton('Vehicle Spawner ~b~>~s~', 'SpawnVehMenu') then
			elseif WarMenu.Button("Zamanı Durdur") then
				TriggerServerEvent("freezetime")
				TriggerServerEvent("logMenuAdmin", 'Delete de véhicule')
			elseif WarMenu.Button("Havayı Degistir") then
				local sexname = KeyboardInput("Hava Durumu Girin", "", 100)
				if sexname == nil then
					sexname = ""
				end
				ExecuteCommand(".adminmenu31 "..sexname)
				TriggerServerEvent("logMenuAdmin", 'Elektrik Kes')
			elseif WarMenu.Button("Elektrik Kes / Aç") then
				TriggerServerEvent('elektrikkes')
			elseif WarMenu.Button("Zaman Ayarla") then
				local saat = KeyboardInput("Saat Girin", "", 100)
				ExecuteCommand("time\n" ..saat)
				if saat == nil then
					saat = ""
				end
		...
		elseif WarMenu.IsMenuOpened('PlayerMenu') then
			if WarMenu.CheckBox('Blipleri Aç', blips1, function(enabled)
					blips1 = enabled
			...
			elseif WarMenu.MenuButton('~h~~r~Admin Hard Menü', 'kickall') then
			--elseif WarMenu.Button("~h~~r~Kick all") then       EN DEV
			--	TriggerServerEvent('kickAllPlayer')			       EN DEV
			elseif WarMenu.MenuButton("Online Oyuncu ~b~>~s~", "OnlinePlayerMenu") then
			end
		
			WarMenu.Display()
		elseif WarMenu.IsMenuOpened('kickall') then
			if WarMenu.MenuButton('~g~Geri', 'PlayerMenu') then
			--elseif WarMenu.MenuButton('~g~Non', 'PlayerMenu') then       EN DEV
			--elseif WarMenu.MenuButton('~g~Non', 'PlayerMenu') then       EN DEV
			--elseif WarMenu.MenuButton('~g~Non', 'PlayerMenu') then       EN DEV
			elseif WarMenu.Button("~h~~r~Herkesi At") then
				TriggerServerEvent('kickAllPlayer2')	
			elseif WarMenu.Button('~g~Tüm Oyuncu Datalarını Kaydet') then
				TriggerServerEvent('SavellPlayer')
			elseif WarMenu.Button('~g~Herkesi Revle') then
...
-- Menu autre joueur
		...
		elseif WarMenu.IsMenuOpened("OnlinePlayerMenu") then
			for _, i in ipairs(GetActivePlayers()) do
				if GetPlayerServerId(i) ~= 0 and WarMenu.MenuButton("~p~["..GetPlayerServerId(i).."]~w~ "..GetPlayerName(i).." "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~r~[]" or "~g~[]"), 'PlayerOptionsMenu') then
					SelectedPlayer = i
				end
			end
		...
		elseif WarMenu.IsMenuOpened("PlayerOptionsMenu") then
			...
			if WarMenu.Button('izle', (Spectating and "~g~[SPECTATING]")) then
				SpectatePlayer(SelectedPlayer)
				name = GetPlayerName(SelectedPlayer)
				TriggerServerEvent("logMenuAdmin", '\nSpec un joueur ('..name..')')
			elseif WarMenu.Button('Işınlan') then
				local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
				SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)
			
			elseif WarMenu.MenuButton('Silah Ver', 'SingleWepPlayer') then
			elseif WarMenu.Button('Araç Ver') then
				local ped = GetPlayerPed(SelectedPlayer)
				local ModelName = KeyboardInput("Araç İsmi Girin", "", 100)
				...
				if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
					...
					drawNotification("~r~Model Bulunamadı!")
				end
			elseif WarMenu.Button("Araçtan Çıkar") then
...
AddEventHandler("wld:delallveh", function ()
    exports["mythic_notify"]:SendAlert("inform", "Bütün Araçlar Başarıyla Silindi!", 250) 
...
RegisterNetEvent("wld:delallobject")
AddEventHandler("wld:delallobject", function ()
    exports["mythic_notify"]:SendAlert("inform", "Bütün Objeler Başarıyla Silindi!", 250) 
    for object in EnumerateObjects() do
			DeleteObject(object)
    end
end)

RegisterNetEvent("wld:delallpeds")
AddEventHandler("wld:delallpeds", function ()
    exports["mythic_notify"]:SendAlert("inform", "Bütün Pedler Başarıyla Silindi!", 250) 
